...
import com.ping.syncparse.sync.c9.DocumentRevenueEntity;
import com.ping.syncparse.sync.c9.DocumentRevenueMapper;
...
public class SearchService1 {
    ...
    @Autowired
    private DocumentRevenueMapper documentRevenueMapper;
    ...
    private void convert(BaseEntity entity){
        if (StringUtils.hasLength(entity.getCaseNo()) && StringUtils.hasLength(entity.getName()) && entity.getRefereeDate() != null) {
            DocumentRevenueEntity xsEntity = new DocumentRevenueEntity();
            ...
            try {
                documentRevenueMapper.insert(xsEntity);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }

...
import com.ping.syncparse.sync.c9.DocumentFalseInvoiceEntity;
import com.ping.syncparse.sync.c9.DocumentFalseInvoiceMapper;
...
public class SearchService5 {
    ...
    @Autowired
    private DocumentFalseInvoiceMapper documentFalseInvoiceMapper;
    ...
    private void convert(BaseEntity entity) {
        if (StringUtils.hasLength(entity.getCaseNo()) && StringUtils.hasLength(entity.getName()) && entity.getRefereeDate() != null) {
            DocumentFalseInvoiceEntity xsEntity = new DocumentFalseInvoiceEntity();
            ...
            try {
                documentFalseInvoiceMapper.insert(xsEntity);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }

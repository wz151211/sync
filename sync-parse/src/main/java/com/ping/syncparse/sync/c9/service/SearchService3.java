...
import com.ping.syncparse.sync.c9.DocumentFinanceMapper;
...
public class SearchService3 {
    ...
    @Autowired
    private DocumentFinanceMapper documentFinanceMapper;
    ...
    private void convert(BaseEntity entity) {
        if (StringUtils.hasLength(entity.getCaseNo()) && StringUtils.hasLength(entity.getName()) && entity.getRefereeDate() != null) {
            DocumentFinanceEntity xsEntity = new DocumentFinanceEntity();
            ...
            try {
                documentFinanceMapper.insert(xsEntity);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
